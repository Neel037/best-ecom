...
const Products = () => {
  ...
  const fetchProducts = (category) => {
    setIsLoading(true);
    let url = 'https://api.storerestapi.com/products?limit=12&page=1';
    if (category !== 'all') {
  ...
  return (
    <>
      ...
      <Container className="my-4 pt-5" id='products'>
      <Navbr categories={['all', ...categories.map(cat => cat.name)]} onSelectCategory={handleSelectCategory} />
        <Row>
          {data.map(item => (
            <Col md={4} key={item.id} className="mb-4">
              <Card>
                <Card.Img variant="top" src={`https://picsum.photos/300/200?random=${Math.random()}`} />
                <Card.Body>
